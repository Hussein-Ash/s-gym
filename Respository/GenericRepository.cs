...
namespace EvaluationBackend.Repository
{
    public class GenericRepository<T, TId> : IGenericRepository<T, TId> where T : BaseEntity<TId>
    {
        ...
        {
            // return await _dbContext.Set<T>().Where(e => e.Id == id ).ProjectTo<TDto>(_mapper.ConfigurationProvider);
        ...
        {
            var query = _dbContext.Set<T>().Where(w => w.Deleted == false).AsNoTracking();
        ...
        private async Task<(List<TDto> data, int totalCount)> ExecuteQuery<TDto>(IQueryable<T> query, int pageNumber, int pageSize)
        {
            ...
            : await query.Skip(pageSize * (pageNumber - 1)).Take(pageSize).OrderByDescending(x => x.CreationDate).ProjectTo<TDto>(_mapper.ConfigurationProvider).ToListAsync();
        ...
        public async Task<T> Delete(TId id)
        {
            ...
            if (result == null || result.Deleted == true) return null;
